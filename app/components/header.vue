<template>
  <header :class="[{ '-translate-y-full': isHidden }, 'bg-mcs-blue flex md:justify-center justify-between items-center lg:px-auto px-5', 'sticky top-0 left-0 right-0 z-40 transform transition-transform duration-300']" :aria-hidden="isHidden ? 'true' : 'false'">
        <div class="flex py-4 justify-between items-center xl:w-[1180px] lg:w-[940px] w-full">
            <NuxtLink :to="$localePath('/')"><img :src="logoHeader" alt="MCS logo"></NuxtLink>
            <nav class="hidden sm:flex space-x-6 items-center">
                <NuxtLink :to="$localePath('/about')" @click="isOpen = false" :class="linkClass('/about')" >{{ $t('header.about') }}</NuxtLink>
                <NuxtLink :to="$localePath('/projects')" @click="isOpen = false" :class="linkClass('/projects')" >{{ $t("header.projects") }}</NuxtLink>
                <NuxtLink :to="$localePath('/partners')" @click="isOpen = false" :class="linkClass('/partners')" >{{ $t("header.partners") }}</NuxtLink>
                <NuxtLink :to="$localePath('/support-us')" @click="isOpen = false" :class="linkClass('/support-us')" >{{ $t("header.support-us") }}</NuxtLink>
                <NuxtLink :to="$localePath('/articles')" @click="isOpen = false" :class="linkClass('/articles')" >{{ $t("header.articles") }}</NuxtLink>
                <button
        ...
        <teleport to="body">
          <div v-if="isOpen" id="mobile-menu" class="fixed inset-0 z-50 sm:hidden bg-mcs-blue" role="dialog" aria-modal="true">
              <div class="flex items-center justify-between px-5 py-4">
                <div>
                  <NuxtLink :to="$localePath('/')" @click="isOpen = false"><img :src="logoHeader" alt="MCS logo"></NuxtLink>
                </div>
              ...
              <nav class="h-4/5 flex flex-col items-center justify-center gap-8">
                  <NuxtLink :to="$localePath('/about')" @click="isOpen = false" :class="linkClass('/about')" class="text-2xl">{{ $t('header.about') }}</NuxtLink>
                  <NuxtLink :to="$localePath('/projects')" @click="isOpen = false" :class="linkClass('/projects')" class="text-2xl">{{ $t('header.projects') }}</NuxtLink>
                  <NuxtLink :to="$localePath('/partners')" @click="isOpen = false" :class="linkClass('/partners')" class="text-2xl">{{ $t('header.partners') }}</NuxtLink>
                  <NuxtLink :to="$localePath('/support-us')" @click="isOpen = false" :class="linkClass('/support-us')" class="text-2xl">{{ $t('header.support-us') }}</NuxtLink>
                  <NuxtLink :to="$localePath('/articles')" @click="isOpen = false" :class="linkClass('/articles')" class="text-2xl">{{ $t('header.articles') }}</NuxtLink>
                  <button
