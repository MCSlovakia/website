...
async function loadArticlesForLocale(locale) {
  const contentDir = path.join(rootDir, 'content', 'projects', locale);
  let entries = [];
  try {
...
async function main() {
  const locales = ['sk', 'en'];
  const outDir = path.join(rootDir, 'app', 'data', 'projects');
  ...
  for (const locale of locales) {
    ...
    console.log(`Wrote ${articles.length} ${locale.toUpperCase()} projects to`, outPath);
  }
}
